...
/** This is a resharing simulator written for Malicious Sort
 *  Current implementation of CECILIA doesn't allow role change, here we are simulating the role chage and reharing of the shares
 *  Communication and computation cost is the same with the original idea.
 *  Check the sorting paper for more details
 * */
uint64_t *Reshare(Party *const proxy, const uint64_t *const a, uint32_t sz) {

    if ( proxy->GetPRole() == proxy1) {
        auto a_new = new uint64_t[sz];
        ...
        for (int i = 0; i < sz; i++) {
            ...
            tempShare2 = proxy->GenerateRandom();
            AddValueToCharArray(tempShare, &ptr1);     // for P0
            AddValueToCharArray(tempShare2, &ptr2);   //P1 share
            a_new[i] = a[i] - tempShare2;
        }
        thread thr1 = thread(Send, proxy->GetSocketHelper(), proxy->GetBuffer1(), sz * 8);
        thread thr2 = thread( Send, proxy->GetSocketP2(), proxy->GetBuffer2(), sz * 8);
        thr1.join();
        thr2.join();

        return a_new;
    }
    else if (proxy->GetPRole() == proxy2) { //P0 or proxy1
        unsigned char *ptr = proxy->GetBuffer1();
        unsigned char *ptr2 = proxy->GetBuffer2();
        uint64_t *a_new = new uint64_t[sz];  //a+r_i

        Receive(proxy->GetSocketP1(),proxy->GetBuffer1(),sz*8);   // receive XOR share of (a+r)

        uint64_t tempShare;
        for (int i = 0; i < sz; i++) {
            tempShare = proxy->GenerateRandom();
            AddValueToCharArray(tempShare, &ptr2);
            a_new[i] = a[i] + ConvertToLong(&ptr);
        }
        Send(proxy->GetSocketHelper(), proxy->GetBuffer2(), sz * 8);  //sent ar to helper
        ...
        unsigned char *ptr = proxy->GetBuffer1();
        unsigned char *ptr2 = proxy->GetBuffer2();
        uint64_t *r = new uint64_t[sz];  //a+r_i

        Receive(proxy->GetSocketP1(),proxy->GetBuffer1(),sz*8);   // receive XOR share of (a+r)
        Receive(proxy->GetSocketP2(),proxy->GetBuffer2(),sz*8);   // receive XOR share of (a+r)

        for (int i = 0; i < sz; i++) {
            r[i] = ConvertToLong(&ptr2) + ConvertToLong(&ptr);
        }
...
__uint128_t *Reconstruct128Bit(Party *const proxy, const __uint128_t *const a, uint32_t sz) {
    if (proxy->GetPRole() != helper) {
        __uint128_t *b = new __uint128_t[sz];
        if (proxy->GetPRole() == proxy1 ) {
            unsigned char *ptr = proxy->GetBuffer1();
            for (int i = 0; i < sz; i++) {
		//cout<< "value will be sent " << (uint64_t)a[0] << "\t" << (uint64_t)(a[0]>>64) <<endl; 
                AddValueToCharArray128Bit(a[i], &ptr);
            }
		//cout<< "value added to buffer " << (uint64_t)a[0] << "\t" << (uint64_t)(a[0]>>64) <<endl; 
            thread thr1 = thread(Send, proxy->GetSocketP2(), proxy->GetBuffer1(), sz * 16);
            thread thr2 = thread(Receive, proxy->GetSocketP2()+SOCKET_NUMBER, proxy->GetBuffer2(), sz * 16);
            thr1.join();
            thr2.join();

            ptr = proxy->GetBuffer2();
            for (int i = 0; i < sz; i++) {
                b[i] = ConvertTo128Bit(&ptr);
            }
		//cout<< "value sent and received " << (uint64_t)a[0] << "\t" << (uint64_t)b[0] <<endl; 
		//cout<< "value sent and received h " << (uint64_t)(a[0]>>64) << "\t" << (uint64_t)(b[0]>>64) <<endl; 
		...
		//cout<< "value added to buffer " << (uint64_t)a[0] << "\t" << (uint64_t)(a[0]>>64) <<endl; 
            ...
            thread thr1 = thread(Send, proxy->GetSocketP1()+SOCKET_NUMBER, proxy->GetBuffer1(), sz * 16);
            thread thr2 = thread(Receive, proxy->GetSocketP1(), proxy->GetBuffer2(), sz * 16);
            thr1.join();
            thr2.join();
            ptr = proxy->GetBuffer2();
            for (int i = 0; i < sz; i++) {
...
uint64_t *Reconstruct(Party *const proxy, const uint64_t *const a, uint32_t sz, uint64_t mask= RING_SIZE) {
    if (proxy->GetPRole() != helper) {
        ...
        if (proxy->GetPRole() == proxy1 ) {
            ...
            thread thr2 = thread(Receive, proxy->GetSocketP2()+SOCKET_NUMBER, proxy->GetBuffer2(), sz * 8);
            thr1.join();
            thr2.join();
            ...
            thread thr1 = thread(Send, proxy->GetSocketP1()+SOCKET_NUMBER, proxy->GetBuffer1(), sz * 8);
 ...
 * @param size the number of multiplication triples that will be generated
 */
void GenerateMultiplicationTriple128Bit(Party *const proxy, __uint128_t *const c1, uint32_t size) {
    for (int i = 0; i < size; i++) {
        __uint128_t a0 = proxy->GenerateCommonRandom();
        __uint128_t a1 = proxy->GenerateCommonRandom2();
        __uint128_t b0 = proxy->GenerateCommonRandom();
        __uint128_t b1 = proxy->GenerateCommonRandom2();
        __uint128_t c0=  proxy->GenerateCommonRandom();
        c1[i] = ((a0+a1)*(b0+b1)) - c0;
    }
}
...
uint32_t* MultiplexNarrow(Party *const proxy, const uint32_t *const x, const uint32_t *const y, const uint32_t *const b, uint32_t sz, uint32_t ringbits) {
    ...
    if (proxy->GetPRole() == proxy1){
        ...
        uint32_t *r1 = new uint32_t[sz];

        for (uint32_t i = 0; i < sz; i++) {
            r1[i] = proxy->GenerateCommonRandomShort();
        }
        uint8_t *ptr = proxy->GetBuffer1();
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = 0; i < sz; i++) {
            uint32_t r2 = r1[i] ^ 0xae074761;
            uint32_t r3 = r1[i] ^ 0x93f430cb;
            uint32_t r4 = r1[i] ^ 0xe701a01d;
            uint32_t xy = (x[i] - y[i]) & mask;

            m1[i] = ((b[i] * xy) - (r2 * b[i]) - (r3 * xy) - (r3 * r4)) & mask;
            uint32_t m2 = (b[i] + r1[i]) & mask;
            uint32_t m3 = (xy + r4) & mask;
            AddValueToCharArray2(m2, ptr+2*i*bsz, bsz);
            AddValueToCharArray2(m3, ptr+2*i*bsz+bsz, bsz);
        }
        delete [] r1;
        Send(proxy->GetSocketHelper(), proxy->GetBuffer1(), sz * bsz * 2);
        uint32_t sz1 = sz/2;
        thread thr1 = thread(Receive,proxy->GetSocketHelper(), proxy->GetBuffer1(), (sz-sz1) * bsz);
        for (uint32_t i = 0; i < sz1; i++) {
            res[i] = (x[i] - m1[i] - proxy->GenerateCommonRandomShort2()) & mask;
        }
        thr1.join();
        ptr = proxy->GetBuffer1();
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = sz1; i < sz; i++) {
            res[i] = (m1[i] + ConvertToLong2(ptr+(i-sz1)*bsz,bsz)) & mask;
...
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = 0; i < sz; i++) {
            ...
            m1[i] = ((b[i] * xy) - (r1[i] * xy) - (r1[i] * r2) - (r4 * b[i])) & mask;
            uint32_t m2 = (xy + r2) & mask;
        ...
        uint32_t sz1 = sz/2;
        thread thr1 = thread(Receive,proxy->GetSocketHelper(), proxy->GetBuffer1(), sz1 * bsz);
        for (uint32_t i = sz1; i < sz; i++) {
            res[i] = (x[i] - m1[i] - proxy->GenerateCommonRandomShort2()) & mask;
        }
        thr1.join();
...
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = 0; i < sz1; i++) {
            res[i] = (m1[i] + ConvertToLong2(ptr+i*bsz,bsz)) & mask;
            res[i] = (x[i] - res[i]) & mask;
        }
        delete [] m1;
        return res;
        ...
        uint32_t *rnd1 = new uint32_t [sz];
        uint32_t *rnd2 = new uint32_t [sz];
        uint32_t sz1 = sz/2;
        for (uint32_t i = 0; i < sz1; i++) {
            rnd1[i] = proxy->GenerateCommonRandomShort() & mask;
        }
        for (uint32_t i = sz1; i < sz; i++) {
            rnd1[i] = proxy->GenerateCommonRandomShort2() & mask;
        }
        thr1.join();
        thr2.join();
...
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = 0; i < sz; i++) {
            uint32_t m2 = ConvertToLong2(ptr + 2 * i * bsz, bsz);
            uint32_t m3 = ConvertToLong2(ptr + 2 * i * bsz + bsz, bsz);
            uint32_t m5 = ConvertToLong2(ptr2 + 2 * i * bsz, bsz);
            uint32_t m6 = ConvertToLong2(ptr2 + 2 * i * bsz + bsz, bsz);
            uint32_t m = ((m2 * m5) + (m3 * m6)) & mask;
            rnd2[i] = (m - rnd1[i]) & mask;
        }
        ptr = proxy->GetBuffer1();
        ptr2 = proxy->GetBuffer2();
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = 0; i < sz1; i++) {
            AddValueToCharArray2(rnd2[i], ptr2 + i * bsz, bsz);
        }
#pragma omp parallel for num_threads(THREAD_NUMBER)
        for (uint32_t i = sz1; i < sz; i++) {
            AddValueToCharArray2(rnd2[i], ptr+(i-sz1)*bsz, bsz);
        }

        delete []rnd1;
        delete []rnd2;
        thr1 = thread(Send, proxy->GetSocketP1(), proxy->GetBuffer1(), (sz-sz1) * bsz);
        thr2 = thread(Send, proxy->GetSocketP2(), proxy->GetBuffer2(), sz1 * bsz);
        thr1.join();
        thr2.join();
        return NULL;
    }
...
uint64_t* Equals(Party *const proxy, const uint64_t *const x, const uint64_t *const y, uint32_t size, int shift = FRACTIONAL_BITS) {
    if (proxy->GetPRole() != helper) {
        ...
        for (int i = 0; i < size; i++) {
            if (proxy->GetPRole() == proxy1) {
                m[i] =  - greater_and_smaller[i] -greater_and_smaller[i+size];
            } else {
                m[i] = ConvertToUint64(1, shift) -greater_and_smaller[i] -greater_and_smaller[i+size];
            }
        }
 ...
 */
__uint128_t *Multiply128Bit(Party *const proxy, const uint64_t *const a, const uint64_t *const b, uint32_t size, uint64_t of = 0, int shift = FRACTIONAL_BITS) {
    if (DEBUG_FLAG >= 1)
        cout << "************************************************************\nPMNF_MUL is called" << endl;
    if (proxy->GetPRole() == helper) {
        __uint128_t *c1 = new __uint128_t[size];

        GenerateMultiplicationTriple128Bit(proxy, c1, size);

        unsigned char *ptr_out2 = proxy->GetBuffer2();
        for (int j = 0; j < size; j++) {
            AddValueToCharArray128Bit(c1[j], &ptr_out2);
        }

        Send( proxy->GetSocketP2(), proxy->GetBuffer2(), size * 16);
        ...
        __uint128_t *mt[3];
        mt[0] = new __uint128_t[size]; //a
        mt[1] = new __uint128_t[size]; //b
        mt[2] = new __uint128_t[size]; //c
        __uint128_t *concat_e_f = new __uint128_t[size * 2];
        if (proxy->GetPRole() == proxy2) {
            Receive(proxy->GetSocketHelper(), proxy->GetBuffer1(), size * 16);
            unsigned char *ptr = proxy->GetBuffer1();
            for (int i = 0; i < size; ++i) {
                mt[0][i] = proxy->GenerateCommonRandom2();
                mt[1][i] = proxy->GenerateCommonRandom2();
                mt[2][i] = ConvertTo128Bit(&ptr);
                concat_e_f[i] = ((__uint128_t)a[i] - mt[0][i] - of);
                concat_e_f[i + size] = ((__uint128_t)b[i] - mt[1][i] - of);
            }
        }
        else {
            for (int i = 0; i < size; ++i) {
    ...
    __uint128_t *e_f = Reconstruct128Bit(proxy, concat_e_f, size * 2);
  //      for(int i = 0; i<3; i++){

       // cout <<"mt0 and mt1 and mt2" << Reconstruct(proxy, mt[0][i]) << "\t" <<  Reconstruct(proxy, mt[1][i]) << "\t"<< Reconstruct(proxy, mt[2][i])<<endl;
       // cout << "ef before rec "<< (uint64_t) concat_e_f[0] << "\t" << (uint64_t) (concat_e_f[0] >>64) << endl;
 
//	cout <<"e0 rec" <<(uint64_t) e_f[i] << "\t" << (uint64_t) (e_f[i] >>64) << endl;
//        cout <<"f0 rec " <<(uint64_t) e_f[size+i] << "\t" << (uint64_t) (e_f[size+i] >>64) << endl;
//}
        __uint128_t *e = e_f;
        __uint128_t *f = &e_f[size];
 ...
 * @return a vector containing the share of the result of the multiplication
 */
uint64_t *MultiplyNarrow(Party *const proxy, const uint64_t *const a, const uint64_t *const b, uint32_t size, uint32_t ringbits, int shift = FRACTIONAL_BITS) {
...
/** Multiplication of two arrays of 64 bit numbers with result of 128 bit
 *
 * @param a one of the vectors of shares of the multiplicands
 * @param b the other vector of shares of the multiplicands
 * @param size the size of the vectors @p a and @p b
 * @result high most significant 64 bit of the multiplication result
 * @result low most significant 64 bit of the multiplication result
 */
void Multiply64to128Bit(Party *const proxy, const uint64_t *const a, const uint64_t *const b, uint64_t * high, uint64_t * low, uint32_t size, int shift = FRACTIONAL_BITS) {
    ...
    if (proxy->GetPRole() == helper) {
        ...
        uint64_t *a_lo = new uint64_t[size];
        uint64_t *a_hi = new uint64_t[size];
        uint64_t *b_hi = new uint64_t[size];
        uint64_t *b_lo = new uint64_t[size];
        uint64_t *w3 = new uint64_t[size];
        uint64_t *k = new uint64_t[size];
        uint64_t *w1 = new uint64_t[size];
        cout << "MUL" << endl;
        cout << Reconstruct(proxy, a[0]) << " \t" << Reconstruct(proxy, b[0]) << endl;
        for (int i = 0; i < size; ++i) {
            a_lo[i] = a[i] & mask;    //uint64 a_lo = (u & 0xffffffff);
            b_lo[i] = b[i] & mask;    //uint64 b_lo = (v & 0xffffffff);
        }
        cout << a_lo[0] << " \t" << b_lo[0] << endl;
        cout << Reconstruct(proxy, a_lo[0]) << " \t" << Reconstruct(proxy, b_lo[0]) << endl;

        auto ab_lo = Multiply(proxy, a_lo, b_lo, size); // uint64 t = (a_lo * b_lo);
        cout << Reconstruct(proxy, ab_lo[0]) << endl;

        for (int i = 0; i < size; ++i) {
            w3[i] = ab_lo[i] & mask;    //uint64 w3 = (t & 0xffffffff);
            k[i] = ArithmeticShift(ab_lo[i], 32);      //uint64 k = (t >> 32);
            a_hi[i] = ArithmeticShift(a[i], 32); //u >>= 32;
        }
        cout << "a\t" << a[0] << endl;
        cout << Reconstruct(proxy, w3[0]) << " \t" << Reconstruct(proxy, k[0]) << "\t" << Reconstruct(proxy, a_hi[0]) << endl;

        //t = (u * b_lo) + k;
        auto t = Multiply(proxy, a_hi, b_lo, size);
        for (int i = 0; i < size; ++i) {
            t[i] = t[i] + k[i];
        }
        cout << Reconstruct(proxy, t[0])  << endl;

        // k = (t & 0xffffffff);
        for (int i = 0; i < size; ++i) {
            k[i] = t[i] & mask;
        }
        cout << Reconstruct(proxy, k[0]) << endl;

        for (int i = 0; i < size; ++i) {
            w1[i] = ArithmeticShift(t[i], 32); //uint64 w1 = (t >> 32);
            b_hi[i] = ArithmeticShift(b[i], 32); // v >>= 32;
        }
        cout << Reconstruct(proxy, w1[0]) << " \t" << Reconstruct(proxy, b_hi[0]) << endl;

        //        t = (a_lo * v) + k;
        t = Multiply(proxy, a_lo, b_hi, size);
        for (int i = 0; i < size; ++i) {
            t[i] = t[i] + k[i];
        }
        cout << Reconstruct(proxy, t[0])  << endl;
        ...
        // k = (t >> 32);
        for (int i = 0; i < size; ++i) {
            k[i] = ArithmeticShift(t[i] , 32);
        }
        cout << Reconstruct(proxy, k[0])  << endl;

//        h = (u * v) + w1 + k;
        auto high_temp = Multiply(proxy, a_hi, b_hi, size);
        for (int i = 0; i < size; ++i) {
            high[i] = high_temp[i] + w1[i]+k[i];
        }
        auto high_rec = Reconstruct(proxy, high, size);
        for (int i = 0; i < size; ++i) {
            if (high_rec[i]!=0){
                cout << "w1 and k\t"<<Reconstruct(proxy, w1[i]) << " \t" << Reconstruct(proxy, k[i]) << endl;
                cout << Reconstruct(proxy, a[i]) << " \t" << Reconstruct(proxy, b[i]) << endl;
                cout << Reconstruct(proxy, a_hi[i]) << " \t" << Reconstruct(proxy, b_hi[i]) << endl;
                cout << "hightemp\t" << Reconstruct(proxy, high_temp[i]) << endl;
                cout << "high\t"<<Reconstruct(proxy, high[i]) << endl;
                break;
            }
        }
 ...
 *
 * */
void LocalMultiply64To128Bit(uint64_t *a, uint64_t *b, uint64_t *high, uint64_t *low, uint32_t size){
    auto mask = 0xffffffff;
        uint64_t *u1 = new uint64_t[size];
        uint64_t *a_temp = new uint64_t[size];
        uint64_t *b_temp = new uint64_t[size];
        uint64_t *v1 = new uint64_t[size];
        ...
        for (int i = 0; i < size; ++i) {
            u1[i] = a[i] & mask;    //uint64 u1 = (u & 0xffffffff);
            v1[i] = b[i] & mask;    //uint64 v1 = (v & 0xffffffff);
            t[i] = u1[i] *v1[i];
        }

        for (int i = 0; i < size; ++i) {
            w3[i] = t[i] & mask;    //uint64 w3 = (t & 0xffffffff);
            k[i] = t[i] >> 32;      //uint64 k = (t >> 32);
            a_temp[i] = a[i] >> 32; //u >>= 32;
        }

        //t = (u * v1) + k;
        for (int i = 0; i < size; ++i) {
            t[i] = a_temp[i] *v1[i]+k[i];
        }
        // k = (t & 0xffffffff);
        for (int i = 0; i < size; ++i) {
            k[i] = t[i] & mask;
        ...
        for (int i = 0; i < size; ++i) {
            w1[i] = (t[i] >> 32); //uint64 w1 = (t >> 32);
            b_temp[i] = (b[i] >> 32); // v >>= 32;
        }
        //        t = (u1 * v) + k;
        for (int i = 0; i < size; ++i) {
            t[i] = u1[i]*b_temp[i] + k[i];
        }
...
//        h = (u * v) + w1 + k;
        for (int i = 0; i < size; ++i) {
            high[i]= a_temp[i]*b_temp[i] + w1[i]+k[i];
        }
        cout << high[0] << endl;
