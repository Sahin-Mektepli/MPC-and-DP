...
//
#include <iostream>
#include "../core/core_full.h"
#include "../core/cnn.h"
#include "../core/rkn.h"
#include "../core/auc.h"
#include "../core/sort.h"
#include "../booleancore/core.h"
#include "../core/heavyhitters.h"
#include "../core/knn.h"
    ...
    while (keep_looping){
        ...
        switch(operation) {
            ...
            case coreVectorisedMultiply2:
                ...
                break;
            case coreVectorisedMultiply128BitOld:
                sz = proxy->ReadInt();
                Multiply64to128Bit(proxy, nullptr, nullptr, nullptr, nullptr,sz);
                break;
            case coreVectorisedMultiply128Bit:
                sz = proxy->ReadInt();
                Multiply128Bit(proxy, nullptr, nullptr,sz);
                break;
            case coreDotProduct:
                sz = proxy->ReadInt();
            ...
            case coreSort:
                ...
                break;
            case coreMaliciousSort:
                sz = proxy->ReadInt();
                size1 = proxy->ReadInt();
                SortMalicious(proxy, nullptr, nullptr, nullptr, sz, size1);
                break;
            case coreGenSortPerm:
                sz = proxy->ReadInt();
                size1 = proxy->ReadInt();
                params[0] = proxy->ReadInt();
                GenerateSortingPermutation(proxy, 0, 0, sz, size1, params[0]);
                break;
            case coreApplySortPerm:
                sz = proxy->ReadInt();
                size1 = proxy->ReadInt();
                ApplyPerm(proxy, 0, 0, 0, sz,  size1);
                break;
            case coreHeavyHitters:
                params[0] = proxy->ReadInt();
                params[1] = proxy->ReadInt();
                params[2] = proxy->ReadInt();
                params[3] = proxy->ReadInt();
                HeavyHitters(proxy, 0, params[0],params[1], params[2], params[3]);
                break;
            case coreKnn:
                params[0] = proxy->ReadInt();
                params[1] = proxy->ReadInt();
                ...
                params[3] = proxy->ReadInt();
                params[4] = proxy->ReadInt();
                KNN(proxy, 0, 0, 0, params[0],params[1], params[2], params[3], params[4]);
                break;
            case boolAnd:
                sz = proxy->ReadInt();
                And2(proxy, 0, 0, sz);
            ...
            case boolArithmeticToXor:
                sz = proxy->ReadInt();
                size1 = proxy->ReadInt();
                ArithmeticToXor(proxy, 0, sz, nullptr, size1);
                break;
