...
int main(int argc, char* argv[]) {
    ...
    if(FRACTIONAL_BITS == 0) {
         ...
         */
        double reconstructed_d = Reconstruct(proxy, d_secret_share);//tabii bunu normalde yapmamak lazım. 
        ...
        */
        ...
        cout << "ed: " << ConvertToDouble(Reconstruct(proxy, ed_secret_shared)) << endl; //why did they convert to double?..
        ...
        int size = 3; //bunu kullanmayı unutmuşuz :p
        double vec1[3] = {4, 2, -2};
        ...
        uint32_t params[1] = {3}; //3 tane eleman var çünkü, vektörün eleman sayısı olmalı yani bu.
        ...
        if (proxy->GetPRole() == proxy1) { //bunu neden yapıyoruz. yalnız kendi payını değiştirsin diye mi?..
